...
def save_rel_triples(data_root, save_root, ent2ids, rel2ids, ids=1):
    rel_triple_file = os.path.join(data_root, os.path.split(
        data_root)[-1] + "_EntityTriples.txt")
    if ids == 2:
        ill_file = os.path.join(data_root, os.path.split(
            data_root)[-1] + "_SameAsLink.txt")
    else:
        ill_file = None
    triples = list()
    ent_ids = dict()
...
def handle_img_features(data_paths, save_root, ent2ids):
    ...
    repeat_num = 0
    for path in data_paths:
        img_id_path = os.path.join(os.path.dirname(
            path), path.split("/")[-2]+"_ImageIndex.txt")
    ...
    savefile = os.path.join(save_root, os.path.split(
        save_root)[-1] + "_id_img_feature_dict.pkl")
    with open(savefile, 'wb') as f_w:
        pickle.dump(ent2feats, f_w)
    print("repeat num {}".format(repeat_num))


def convert_mmkb_to_dbpStyle(dataset_name, data1, data2): #"FB15K_DB15K" FB DB 
    # attr triples
    ...
    handle_attr_triples(os.path.join(data2, os.path.split(data2)[-1] + "_NumericalTriples.txt"), save_root=data_root,
                        ids=2)
    # rel triples
    data1_rel_path = os.path.join(
        data1, os.path.split(data1)[-1] + "_EntityTriples.txt")
    data2_rel_path = os.path.join(
        data2, os.path.split(data2)[-1] + "_EntityTriples.txt")
    ...
    data1_img_path = os.path.join(
        data1, os.path.split(data1)[-1] + "_ImageData.h5")
    data2_img_path = os.path.join(
        data2, os.path.split(data2)[-1] + "_ImageData.h5")
