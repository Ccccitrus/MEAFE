...
class icl_loss(nn.Module):
    ...
    def forward(self, emb, train_links, emb2=None, norm=True):
        ...
        zis = emb[train_links[:, 0]]#head embedding
        # print("zis shape:", zis.shape) #512(batchsize),100
        if emb2 is not None:
            zjs = emb2[train_links[:, 1]]
        else:
            zjs = emb[train_links[:, 1]]
        # print("zjs shape:", zjs.shape) 512,100

        temperature = self.tau
        alpha = self.weight
        n_view = self.n_view #2
        ...
        labels = F.one_hot(torch.arange(start=0, end=batch_size, dtype=torch.int64), num_classes=num_classes).float()#batchsize * num_classes
        labels = labels.to(self.device)

        masks = F.one_hot(torch.arange(start=0, end=batch_size, dtype=torch.int64), num_classes=batch_size) #batchsize * batchsize
