...
class MultiHeadGraphAttention(nn.Module):
    ...
    def forward(self, input, adj):
        ...
        for i in range(self.n_head):#2
            ...
            edge = adj._indices()
            if self.diag:
                h = torch.mul(input, self.w[i])#torch.mul(a, b) 是矩阵a和b对应位相乘
            else:
                h = torch.mm(input, self.w[i])#torch.mm(a, b) 是矩阵a和b矩阵相乘
            ...
            # middle = -self.leaky_relu(edge_h)
            # edge_e = torch.exp((middle.mm(self.a_src_dst[i]).squeeze()))  # edge_e: 1 x E
