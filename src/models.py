...
class MultiModalEncoder(nn.Module):
    ...
    def __init__(self, args,
        ...
        self.n_units = [int(x) for x in self.args.hidden_units.strip().split(",")]#[128,128,128]
        self.n_heads = [int(x) for x in self.args.heads.strip().split(",")]#[2,2]
        self.input_dim = int(self.args.hidden_units.strip().split(",")[0])#128
    ...
    def forward(self,
        ...
        if self.args.w_gcn:
            gph_emb = self.cross_graph_model(self.entity_emb(input_idx), adj)#input_idx[0,1,...,ent_num]
        else:
            gph_emb = None
        if self.args.w_img:
...
class OcrMultiModalEncoder(nn.Module):
    """
    entity embedding: (ent_num, input_dim)
    gcn layer: n_units
    ...
    def __init__(self, args,
                 ent_num,
                 img_feature_dim,
                 ocr_feature_dim = None,
                 char_feature_dim=None,
                 use_project_head=False):
        super(OcrMultiModalEncoder, self).__init__()
        ...
        self.name_fc = nn.Linear(300, char_dim)
        self.char_fc = nn.Linear(char_feature_dim, char_dim)
        self.ocr_fc = nn.Linear(ocr_feature_dim,100)
    ...
    def forward(self,
        ...
        joint_emb = self.fusion([img_emb, att_emb, rel_emb, gph_emb, name_emb, char_emb, ocr_emb])

        return gph_emb, img_emb, rel_emb, att_emb, name_emb, char_emb, joint_emb , ocr_emb


class AllMultiModalEncoder(nn.Module):
    """
    entity embedding: (ent_num, input_dim)
    gcn layer: n_units
    ...
    def __init__(self, args,
                 ...
                 ocr_feature_dim = None,
                 char_feature_dim=None,
                 modaltype_feature_dim = None,
                 use_project_head=False):
        super(AllMultiModalEncoder, self).__init__()
        ...
        self.char_fc = nn.Linear(char_feature_dim, char_dim)
        self.ocr_fc = nn.Linear(ocr_feature_dim,100)
        self.modaltype_fc = nn.Linear(modaltype_feature_dim,100)
    ...
    def forward(self,
        ...
        ocr_emb = self.ocr_fc(ocr_features)
        modaltype_emb = self.modaltype_fc(modaltype_features)
        ...
        joint_emb = self.fusion([img_emb, att_emb, rel_emb, gph_emb, name_emb, char_emb, ocr_emb,modaltype_emb])

        return gph_emb, img_emb, rel_emb, att_emb, name_emb, char_emb, joint_emb , ocr_emb ,modaltype_emb